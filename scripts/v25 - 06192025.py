# -*- coding: utf-8 -*-
"""
Created on Thu Jun 19 10:58:57 2025
...
info['targetColor_end'] = [152, 152, 152] #if I want to change the contrast, make it closer to 190
...
position_labels = ['TL', 'TR', 'BL', 'BR']
position_map = dict(zip(position_labels, vertices))
opposite_map = {
    'TL': 'TR',
    'TR': 'TL',
    'BL': 'BR',
...
def get_position_label(pos):
    for label, coord in position_map.items():
        if coord == pos:
            return label
    return None

def compute_bar_relation(cue, target, bar_orientation):
    if bar_orientation == "horizontal":
        return "same" if cue[1] == target[1] else "other"
    else:
        return "same" if cue[0] == target[0] else "other"
# =============================================================================
# Trials conditions
# =============================================================================
...
n_catch = int(total_trials * 0.25)
n_valid = int(total_trials * 0.63)
n_invalid_same_bar = int(total_trials * 0.06)
n_invalid_adjacent_other_bar = total_trials - (n_catch + n_valid + n_invalid_same_bar)  # ensure total = 100%

trial_conditions = (
    ['catch'] * n_catch +
    ['valid'] * n_valid +
    ['invalid_same_bar'] * n_invalid_same_bar +
    ['invalid_adjacent_other_bar'] * n_invalid_adjacent_other_bar
)
...
for trial_idx in range(total_trials):
    ...
    trial_type = trial_conditions[trial_idx]
    ...
    cue_pos = random.choice(vertices)
    cue_label = get_position_label(cue_pos)
    
    if trial_type == 'valid':
        target_pos = cue_pos
    elif trial_type == 'invalid_same_bar':
        # get all positions on the same bar, excluding the cue itself
        # From adjacent_map, pick an adjacent corner that is NOT on the same bar
        adjacent_options = adjacent_map[cue_label]
        # filter to ensure it's NOT on the same bar
        if bar_orientation == 'horizontal':
            adjacent_options = [lbl for lbl in adjacent_options if lbl[1] != cue_label[1]]
        else:  # vertical
            adjacent_options = [lbl for lbl in adjacent_options if lbl[0] != cue_label[0]]
        target_label = random.choice(adjacent_options)
        target_pos = position_map[target_label]
        
    elif trial_type == 'invalid_adjacent_other_bar':
        if bar_orientation == 'horizontal':
            same_bar_options = [lbl for lbl in position_labels if lbl[1] == cue_label[1] and lbl != cue_label]
        else:
            same_bar_options = [lbl for lbl in position_labels if lbl[0] == cue_label[0] and lbl != cue_label]
        target_label = random.choice(same_bar_options)
        target_pos = position_map[target_label]
        
    elif trial_type == 'catch':
        target_pos = cue_pos  # doesn't matter because the target is not shown

    info['square_pos'] = pygame.Rect(target_pos[0] - info['targetSize'],
                                     target_pos[1] - info['targetSize'],
                                     info['targetSize'] * 2,
                                     info['targetSize'] * 2)
    info['cue_square_pos'] = pygame.Rect(cue_pos[0] - info['cueSize'],
                                         cue_pos[1] - info['cueSize'],
                                         info['cueSize'] * 2,
                                         info['cueSize'] * 2)
    ...
    bar_relation = compute_bar_relation(cue_pos, target_pos, bar_orientation)
