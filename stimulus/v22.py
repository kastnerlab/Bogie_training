...
info['targetColor_end'] = [155, 155, 155] #if I want to change the contrast, make it closer to 190
    ...
    try:
        with open(local_file, mode, newline='') as f:
            ...
            if isinstance(rows, dict):
                writer.writerow(rows)  # Single row
            else:
...
num_blocks = 1000  # Set the desired number of blocks
for block_num in range(1, num_blocks + 1):
    # Determine trial type and block size
    if block_num % 2 == 1:  # Odd block: catch-only
        block_type = "target"
        num_trials = trials_per_block[1]
    else:  # Even block: target-only
        block_type = "catch"
        num_trials = trials_per_block[0]

    print(f"Starting Block {block_num} ({block_type}-only block, {num_trials} trials)")
    
    for trial_idx in range(num_trials):
        print(f"  Block {block_num}, Global Trial {global_trial_number}")

        # Set trial type based on block type
        ...
        '''# Set cue color based on location
        if target_pos[0] < center_x:  # On the left side
            info['cueColor'] = [220, 0, 0]  # Red
        else:  # On the right side
            info['cueColor'] = [0, 0, 255]  # Blue'''
        ...
        fix=(fix_start_time - experiment_start_time) / 1000
        ...
        while running:
            ...
            if change_cue_color:
                ...
                if cue_frame > info['cue_target_interval']: ###CTI phase
                    corr_resp = 0
                    cti.append(info['cue_target_interval']*frameDur) #we can change to nan for catch trials.
                    running = False
                    change_cue_color = False
                    change_target_color=True # this only applies to if trial type isn't catch. 
            else:
                running = False
        ...
        # =============================================================================
        if trial_type == "catch":
            info['release_timeout'] =0   # 400ms for catch trials
        else:
            ...
            if change_target_color:
                ...
                if not target_returned:
                    ...
                    if trial_type == "target":
                        # Target trial: Display changed target color
                        target_draw(info['targetColor_end'])
                    else:
                        # Catch trial: Display unchanged target color
                        target_draw(info['targetColor_start'])
                    lever_show_draw(info['leverColor'])
                    pygame.display.update()
                ...
                for event in pygame.event.get():
                    if event.type == pygame.MOUSEBUTTONUP:
                        ...
                        if trial_type == "target":
                            corr_resp = 1  # Correct response for target trials
                        else:
                            corr_resp = 0  # Incorrect response for catch trials (lever should remain pressed)
                        running = False
                ...
                if down_dur_t_frame > info['release_timeout']:
                    ...
                    up_time = pygame.time.get_ticks()  # Stop the timer
                    rt_in_seconds = (up_time - target_change_time) / 1000  # Reaction time in seconds
                    rt.append(np.nan) # so if the participant didn't let go, it would be <800ms. 
                    
                    if trial_type == "target":
                        corr_resp = 0  # Missed response for target trials
                    else:
                        corr_resp = 1  # Correct response for catch trials (lever held)
                    running = False
            else:
                running = False
        ...
        if arduino_connected and corr_resp == 1:
            # Send first pulse
            arduino.digital[13].write(1)  # Activate pump pulse
            pygame.time.wait(50)  # Keep pulse HIGH for 50ms (adjustable, between 4-60ms)
            arduino.digital[13].write(0)  # Deactivate pulse
            pygame.time.wait(100)  # Inter-pulse interval (adjustable)
        
            # Send second pulse
            arduino.digital[13].write(1)  # Activate second pulse
            pygame.time.wait(50)  # Keep pulse HIGH for 50ms
            arduino.digital[13].write(0)  # Deactivate second pulse
        ...
        if arduino_connected:
            arduino.digital[13].write(0)  # Ensure pump signal is OFF
        ...
        # Record trial data
        trial_data = [
            block_num,  # Use block_num to track the current block
            global_trial_number,
            trial_type,
            taps_down_loc_fix,
            taps_up_loc_fix,
        ...
        while True:
            ...
            if frame_iti > info['iti']:
                break
    print(f"Completed Block {block_num} ({block_type}-only block)")
