...
if arduino_connected:
    if 'Darwin' in platform.system():  # Mac
        ...
        serPort = "COM7" #change to COM3 if needed
...
for trial_idx in range(total_trials):
    ...
    if trial_type == "catch":
        info['cue_target_interval'] = int((1600 / 1000) / frameDur)
    else:
        info['cue_target_interval'] = int((random.randint(500, 1600) / 1000) / frameDur)
    ...
    if trial_type == "catch":
        ...
        catch_response_window = 0.01  # Allow a brief moment to catch accidental clicks (e.g., 100ms)
        ...
        max_target_hold_frames = int((700 / 1000) / frameDur)
        running = True
        down_dur_t_frame = 0
